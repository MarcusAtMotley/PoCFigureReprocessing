...
- **Revelio SAM text explosion**: `samtools view | split` dumps all reads as uncompressed SAM text (~6x BAM size). A 152GB calmd BAM becomes ~900GB of text, filling disk. Fix: split by chromosome regions using `samtools view -b` (stays compressed, ~20-30GB per group). Run revelio in parallel per group, then merge.
- **bcftools mpileup is single-threaded**: The `--threads` flag only controls BAM decompression, not the pileup computation. On a 143GB WGS BAM (2B reads), takes ~8h on one core. Fix: split by chromosome regions, run parallel mpileup+call per region, then `bcftools concat`. Would cut to ~2-3h.
- **Verify script integrity after Docker extraction**: revelio.py was corrupted by a failed `docker cp` (first line became `/usr/bin/cat: cannot execute binary file`). All 6 revelio processes failed with SyntaxError. Always verify extracted scripts before running.
- **HT29_02N final resolution (v9)**: After 8 OOM failures on 60GB Batch containers, succeeded on local EC2 with 92GB RAM. Full pipeline: markdup → calmd → revelio (6 chromosome groups in parallel) → bcftools mpileup+call. Total ~26.7h, 5,404,221 variants.
